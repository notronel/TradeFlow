  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
      <header className="text-center space-y-2">
        ...
        <p className="text-slate-500 font-medium text-lg">Daily Performance & Rich Narrative</p>
      </header>

      {/* Rich Text Daily Entry Composer */}
      <section 
        ref={composerRef}
        className={`bg-slate-900 border transition-all duration-300 rounded-3xl p-4 shadow-2xl relative ${
          editingId ? 'border-amber-500 ring-2 ring-amber-500/10' : 'border-slate-800'
        }`}
      >
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
            ...
            {editingId && (
              <button onClick={() => { if(editorRef.current) editorRef.current.innerHTML = ''; setEditingId(null); }} className="text-xs text-slate-500 hover:text-white flex items-center gap-1">
                <X size={14} /> Cancel
              </button>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-1 bg-slate-950/50 p-2 rounded-2xl border border-slate-800/50">
          ...
          <div 
            ref={editorRef}
            contentEditable
            className="w-full bg-slate-950/30 min-h-[300px] rounded-2xl p-6 text-base leading-relaxed focus:ring-1 focus:ring-indigo-500/30 outline-none transition-all text-slate-200 empty:before:content-[attr(data-placeholder)] empty:before:text-slate-700 editor-content"
          ...
          <div className="flex items-center justify-between px-2">
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className="p-2.5 rounded-xl text-slate-500 hover:bg-slate-800 transition-all"
              >
                <RotateCcw size={18} />
              </button>
              <button 
      ...
      <div className="space-y-12 pb-24">
        {dailyGroups.map(([date, data]) => (
          <article key={date} className="relative group">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl">
              <div className="flex flex-col md:flex-row items-stretch md:items-center border-b border-slate-800/50">
                <div className="bg-slate-800/30 p-6 flex flex-col items-center justify-center min-w-[140px] border-r border-slate-800/50">
                ...
                <div className="flex-1 p-6 grid grid-cols-2 md:grid-cols-3 gap-6">
                  <Stat label="Total PnL" value={formatCurrencyPlain(data.pnl)} color={data.pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'} />
                  <Stat label="Trades" value={data.count.toString()} color="text-slate-300" />
              ...
              <div className="p-8">
                {data.journalEntry ? (
                  ...
                  <div className="py-4 flex items-center gap-3 text-slate-600 italic text-sm">
                    <History size={14} />
        ...
        {dailyGroups.length === 0 && (
          <div className="text-center py-24 bg-slate-900/20 rounded-3xl border border-dashed border-slate-800">
            ...
            <p className="text-slate-600 italic">"No journal records found."</p>
          </div>
        )}
      </div>

      <style>{`
        .editor-content h1 { font-size: 2rem; font-weight: 800; margin-bottom: 1rem; color: #818cf8; }
        .editor-content h2 { font-size: 1.5rem; font-weight: 700; margin-bottom: 0.75rem; color: #a5b4fc; }
...
const ToolbarBtn = ({ onClick, icon, title }: { onClick: () => void, icon: React.ReactNode, title: string }) => (
  <button 
    ...
    title={title}
    className="p-2 rounded-lg hover:bg-indigo-600/20 text-slate-400 hover:text-indigo-400 transition-all"
  >
    {icon}
  </button>
);
